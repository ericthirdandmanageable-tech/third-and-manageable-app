...
const ArtifactShell = ({
    ...
    return (
        <div className="flex flex-col gap-3">
            <div
                ref={cardRef}
                className="bg-bg-surface rounded-[20px] border border-border-subtle p-8 grain relative overflow-hidden"
            >
                {children}
            ...
            <div className="flex gap-2">
                ...
                >
                    {exported ? (
                        <>
                            <Check className="w-4 h-4 text-volt" /> Saved
                        </>
                    ) : (
                        <>
                ...
                {share && (
                    <button
                        onClick={() => {
                            putDraft(share);
                            router.push(`/community/${share.forumId}`);
                        }}
...
export const DayCounterCard = ({
    ...
    <ArtifactShell name="day-counter">
        ...
        <p className="font-mono text-[11px] uppercase tracking-widest text-text-tertiary mb-4">
            {phaseName}
        </p>
        <p className="font-serif text-xl text-sand italic">
            &ldquo;You showed up today. That&apos;s what matters.&rdquo;
        </p>
    </ArtifactShell>
);
...
export const WeeklyRecapCard = ({
    ...
    <ArtifactShell name="weekly-recap">
        ...
        <div className="flex items-baseline gap-6 mb-4">
            ...
            <p className="font-mono text-5xl text-volt leading-none">
                {completed}/{total}
            </p>
            <p className="font-mono text-[11px] uppercase tracking-widest text-text-tertiary">
                reps done
