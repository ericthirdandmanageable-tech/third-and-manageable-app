...
const AuthModal = ({ onClose }: { onClose: () => void }) => {
    ...
    const submit = async (e: React.FormEvent) => {
        ...
        if (ok) {
            ...
            if (mode === "register") {
                ...
                if (gp && !gp.intake_done) router.push("/onboarding");
            }
        } else {
            setError(
                mode === "login"
    ...
    return (
        <div
            ...
            className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-rise"
        >
            <div className="bg-bg-surface border border-border-subtle rounded-[20px] w-full max-w-md p-6 md:p-8 grain relative">
                ...
                <p className="text-[13px] text-text-secondary mb-6">
                    {mode === "login"
                        ? "A daily game plan and a community that has your back."
                ...
                <p className="text-center text-[13px] text-text-secondary mt-4">
                    ...
                    <button
                        ...
                        className="font-semibold text-volt hover:underline underline-offset-4"
                    >
