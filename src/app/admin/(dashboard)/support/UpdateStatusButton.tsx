...
export default function UpdateStatusButton({
  ...
  const statuses = requestType === "peer" ? STATUSES : ["pending", "resolved"];
  const currentIdx = statuses.indexOf(currentStatus);
  const nextStatus = statuses[currentIdx + 1];

  if (!nextStatus)
    return <span className="text-xs text-green-400">✓ Resolved</span>;
  ...
  const handleUpdate = async () => {
    ...
    try {
      await fetch("/api/update-support", {
        ...
        body: JSON.stringify({ requestId, status: nextStatus, requestType }),
      });
      router.refresh();
    } catch (err) {
