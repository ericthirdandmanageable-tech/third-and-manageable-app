...
function getDatabaseUrl(): string {
  ...
  if (!url) {
    throw new Error(
      "A Neon database URL is not set. The Vercel Neon integration injects POSTGRES_URL_NON_POOLING for identity transactions; run `vercel env pull` for local development (see env.example).",
    );
  }
        ...
        return await db.transaction(async (databaseTransaction) => {
          const transaction: CanonicalIdentityTransaction = {
            ...
            async findBridgeIdentities(providerAccountId) {
              const rows = await databaseTransaction
                .select({
                  ...
                  providerAccountId: authIdentities.providerAccountId,
                ...
                .where(
                  and(
                    ...
                    inArray(authIdentities.provider, BRIDGE_IDENTITY_PROVIDERS),
                  ),
                );
            ...
            async createCanonicalUser() {
              ...
              if (!created) {
                throw new Error("Canonical user creation returned no row");
              }
