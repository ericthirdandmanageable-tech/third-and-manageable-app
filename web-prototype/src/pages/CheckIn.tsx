...
import { Moon, Activity, Heart, ChevronRight, CheckCircle2, Flame, Pencil, Compass, Target, Sparkles, Users } from 'lucide-react';
import clsx from 'clsx';
import { getTodaysPrompt } from '../data/checkin';
...
const CheckIn = () => {
  ...
  const handleSubmit = async () => {
    ...
    const result = editing
      ? (await editToday(selectedOption, journalText || undefined)) ? 'saved' : 'already'
  ...
  return (
    <div className="p-6 md:p-10 max-w-3xl mx-auto animate-rise">
      ...
      {!user && (
        ...
        >
          <p className="font-mono text-[11px] uppercase tracking-widest text-volt mb-2">New here?</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        {AMBIENT_METRICS.map((metric) => (
          <div key={metric.id} className="bg-bg-surface rounded-2xl p-4 border border-border-subtle">
            <div className="flex items-center gap-2 mb-2">
              <div className={`w-8 h-8 rounded-lg ${metric.tile} flex items-center justify-center`}>
                <metric.icon className={`w-4 h-4 ${metric.text}`} />
              </div>
              <span className="font-mono text-[11px] uppercase tracking-widest text-text-tertiary">{metric.label}</span>
            </div>
      ...
      {loading ? (
        <div className="bg-bg-surface rounded-[20px] border border-border-subtle p-10 grain text-center">
          <p className="font-mono text-[11px] uppercase tracking-widest text-text-tertiary animate-pulse">Loading today…</p>
        </div>
      ) : done && !justSubmitted ? (
        ...
        <div className="bg-bg-surface rounded-[20px] border border-border-subtle p-6 md:p-8 grain animate-rise">
          <div className="flex items-start gap-4 mb-5">
            ...
            <div>
              <h2 className="text-xl font-semibold text-text-primary leading-snug">Checked in for today</h2>
          ...
          <div className="bg-bg-elevated border border-border-subtle rounded-2xl px-5 py-4 mb-3">
            <p className="font-mono text-[10px] uppercase tracking-widest text-text-tertiary mb-1">You said</p>
            <p className="text-[15px] text-text-primary">{today?.option}</p>
          </div>
          {today?.journal && (
            <div className="bg-bg-elevated border border-border-subtle rounded-2xl px-5 py-4 mb-3">
              <p className="font-mono text-[10px] uppercase tracking-widest text-text-tertiary mb-1">Journal</p>
              <p className="text-[15px] text-text-secondary leading-relaxed">{today.journal}</p>
            </div>
          )}
          <button
            onClick={startEdit}
          ...
          <div className="mt-6 pt-6 border-t border-border-subtle">
            ...
            >
              ...
              <p className="text-[15px] font-semibold text-text-primary mb-0.5">{nudge.title}</p>
              ...
              <span className="inline-flex items-center gap-1.5 text-[13px] text-volt font-medium">
                {nudge.cta} <ChevronRight className="w-4 h-4 group-hover:translate-x-0.5 transition-transform" />
              </span>
            </button>
          </div>
        </div>
        ...
        <div className="bg-bg-surface rounded-[20px] border border-border-subtle p-10 text-center grain animate-rise">
          ...
          <p className="text-text-secondary mb-8">
            ...
            <span className="font-mono text-volt">{streak}</span> {streak === 1 ? 'day' : 'days'}.
          </p>
          <button
            onClick={() => navigate(nudge.to)}
            className="w-full text-left bg-volt/10 border border-volt/30 rounded-2xl p-5 hover:border-volt/60 hover:bg-volt/15 transition-all group"
        ...
        <div className="bg-bg-surface rounded-[20px] border border-border-subtle p-6 md:p-8 grain">
          <div className="flex items-start gap-4 mb-6">
            ...
            <div>
              <h2 className="text-xl font-semibold text-text-primary leading-snug">{prompt.question}</h2>
              {editing && (
                <p className="font-mono text-[11px] uppercase tracking-widest text-volt mt-1">Editing today's check-in</p>
              )}
            </div>
          </div>
              ...
              >
                <div
                  className={clsx(
                    ...
                    selectedOption === option ? 'border-volt' : 'border-text-tertiary'
                  )}
                >
                  {selectedOption === option && <div className="w-2.5 h-2.5 rounded-full bg-volt" />}
                </div>
          ...
          {selectedOption && (
            <div className="mt-6 pt-6 border-t border-border-subtle animate-disclosure">
              <label className="block text-[13px] font-medium text-text-secondary mb-2">
                Want to say more about that? <span className="text-text-tertiary">(Optional)</span>
              </label>
              <textarea
                value={journalText}
              ...
              <div className="flex items-center gap-3 mt-4">
                ...
                >
                  {editing ? 'Save changes' : 'Save Check-in'} <ChevronRight className="w-5 h-5" />
                </button>
                {editing && (
                  <button
                    onClick={() => { setEditing(false); setSelectedOption(null); setJournalText(''); }}
