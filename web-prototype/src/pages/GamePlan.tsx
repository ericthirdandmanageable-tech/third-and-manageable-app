...
import { Check, ChevronRight, ChevronDown, Sparkles, Lock, RefreshCw } from 'lucide-react';
import clsx from 'clsx';
import IntakeFlow from '../components/IntakeFlow';
import { useGamePlan } from '../hooks/useGamePlan';
import { WORK_PATHS, getPath } from '../data/paths';
    ...
    <div className="p-6 md:p-10 max-w-3xl mx-auto animate-rise">
      ...
      <div className="bg-bg-surface rounded-2xl border border-border-subtle p-5 mb-8">
        <div className="flex justify-between font-mono text-[11px] uppercase tracking-widest text-text-tertiary mb-3">
          {JOURNEY_PHASES.map((p) => (
            <span key={p.id} className={p.id === data.phase.id ? 'text-volt' : undefined}>{p.name}</span>
          ))}
        </div>
        <div className="h-2 bg-bg-elevated rounded-full overflow-hidden">
          <div className="h-full bg-volt rounded-full animate-fill" style={{ width: `${journeyPct}%` }} />
        </div>
      ...
      <section className="bg-bg-surface rounded-[20px] border border-border-subtle p-6 md:p-8 mb-8 grain">
        <div className="flex items-center gap-3 mb-1">
          ...
          <h2 className="text-xl font-semibold text-text-primary">What the game taught you</h2>
        </div>
        ...
        {!data.intakeDone && !intakeOpen && (
          <div className="text-center py-6">
            ...
            <p className="text-text-secondary text-[15px] mb-6 max-w-sm mx-auto">
              Two minutes of guided prompts unlocks your Transferable Skill Map — your sport, in civilian language.
            </p>
            <button
        ...
        {intakeOpen && (
          <IntakeFlow
            onBackAtStart={data.intakeDone ? () => { setIntakeOpen(false); setSearchParams({}); } : undefined}
            onComplete={(answers) => {
        ...
        {data.intakeDone && !intakeOpen && (
          <>
            <div className="flex items-baseline justify-between mb-6">
              <p className="text-[13px] text-text-secondary">Your transferable skill map — in civilian language.</p>
              <button
                onClick={() => setIntakeOpen(true)}
            ...
            <div className="space-y-3">
              {data.skillMap.map((entry) => (
                <div key={entry.skill} className="flex items-center gap-3 flex-wrap">
                  ...
                  <span className="text-[15px] text-text-primary">{entry.translation}</span>
                  {entry.origin && (
                    <span className="font-mono text-[11px] text-text-tertiary w-full pl-1">— {entry.origin}</span>
                  )}
                </div>
              ))}
            </div>
      ...
      <section className="mb-8">
        <div className="flex items-baseline justify-between mb-4">
          ...
          >
            Why this ranking <ChevronDown className={clsx('w-4 h-4 transition-transform', whyOpen && 'rotate-180')} />
          </button>
        </div>
        ...
        {whyOpen && (
          <div className="bg-bg-surface rounded-2xl border border-border-subtle p-5 mb-4 animate-disclosure">
            <p className="text-[13px] text-text-secondary leading-relaxed mb-3">
              Rankings come from your intake and check-in answers — the model is transparent. Contributing signals:
            </p>
            <ul className="space-y-2 text-[13px] text-text-secondary">
              <li className="flex gap-2"><span className="text-volt">·</span> Intake role &amp; favorite part drive the base score</li>
              <li className="flex gap-2"><span className="text-volt">·</span> "Missing the structured schedule" → boosts 9–5 / Corporate</li>
              <li className="flex gap-2"><span className="text-volt">·</span> "The competition itself" → boosts Entrepreneurship</li>
            </ul>
          </div>
        )}
        ...
        <div className="space-y-3">
          {data.pathFit.map((path) => {
            const Icon = WORK_PATHS.find((p) => p.id === path.id)?.icon;
            return (
            <button
              key={path.id}
              onClick={() => navigate(`/game-plan/paths/${path.id}`)}
            ...
            >
              <div className="flex items-start gap-4">
                <div className="w-12 h-12 rounded-lg bg-volt/10 flex items-center justify-center shrink-0">
                  {Icon && <Icon className="w-6 h-6 text-volt" />}
                </div>
                  ...
                  <div className="flex items-center justify-between gap-2 mb-1">
                    ...
                    <span
                      className={clsx(
                        ...
                        path.fit === 'STRONG FIT' ? 'bg-volt/10 text-volt' : 'bg-bg-elevated text-text-tertiary'
                      )}
                    >
                      {path.fit}
                    </span>
                  </div>
                  <p className="text-[13px] text-text-secondary mb-1.5">{path.rationale}</p>
                  <p className="font-mono text-[11px] text-text-tertiary">{path.meta}</p>
                </div>
              </div>
            </button>
            );
      ...
      <section className="bg-bg-surface rounded-[20px] border border-border-subtle p-6 md:p-8">
        <div className="flex items-baseline justify-between mb-1">
          <h2 className="text-xl font-semibold text-text-primary">This Week's Actions</h2>
          <span className="font-mono text-[11px] text-text-tertiary">
            <span className="text-volt">{data.completedActionIds.length}</span> / {data.weeklyActions.length}
          </span>
        </div>
        <p className="text-[13px] text-text-secondary mb-6">Small wins stacked. Same way you trained.</p>
        <div className="space-y-3">
            ...
            return (
              <button
                ...
                className={clsx(
                  ...
                  done ? 'bg-volt/10 border-volt/40' : 'bg-bg-elevated border-border-subtle hover:border-text-tertiary'
                )}
              >
                <div className={clsx('w-5 h-5 rounded-full border flex items-center justify-center shrink-0 transition-colors', done ? 'border-volt bg-volt' : 'border-text-tertiary')}>
                ...
                <div className="flex-1">
                  ...
                  <span className={clsx('text-[15px]', done ? 'text-text-secondary line-through' : 'text-text-primary')}>
                    {action.text}
                  </span>
                </div>
              </button>
