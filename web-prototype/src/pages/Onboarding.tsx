...
const STATUS_OPTIONS: { value: Status; label: string }[] = [
  ...
  { value: 'transitioning', label: "I've transitioned or am transitioning out of competitive sport." },
];
...
const Onboarding = () => {
  ...
  // if they haven't started the flow (the check races the first interactions).
  const stepRef = useRef(step);
  stepRef.current = step;
  useEffect(() => {
    if (loading || !user) return;
    api.getGamePlan().then((gp) => {
      if (gp?.intake_done && stepRef.current === 'welcome') navigate('/', { replace: true });
    });
  }, [user, loading, navigate]);

  // The 4 core steps, in original-app order. Frozen once auth finishes
  // loading so the "Step N of M" numbering doesn't shift mid-flow when an
  // anonymous user creates their account at the account step.
  const frozenSteps = useRef<Step[] | null>(null);
  if (!loading && frozenSteps.current === null) {
    frozenSteps.current = user
      ? ['status', 'intake', 'community']
      : ['status', 'intake', 'account', 'community'];
  }
  const coreSteps: Step[] = frozenSteps.current ?? ['status', 'intake', 'account', 'community'];
  ...
  const backNextRow = (onBack: () => void, onNext: () => void, canNext: boolean, nextLabel = 'Next') => (
    ...
    <div className="min-h-screen bg-bg-base flex items-center justify-center p-6">
      ...
      {step === 'welcome' && (
        <div className="w-full max-w-xl text-center animate-rise">
          <p className="font-mono text-[11px] uppercase tracking-widest text-volt mb-6">Third &amp; Manageable</p>
      ...
      {step === 'status' &&
        stepShell(
          <>
            <h2 className="font-serif text-2xl text-sand italic mb-6">Where are you in your journey?</h2>
                ...
                >
                  <div
                    className={clsx(
                      ...
                      status === option.value ? 'border-volt' : 'border-text-tertiary'
                    )}
                  >
                    {status === option.value && <div className="w-2.5 h-2.5 rounded-full bg-volt" />}
                  </div>
        ...
        stepShell(
          <>
            ...
            <p className="text-[13px] text-text-secondary mb-6">
              Your display name and school help us connect you with the right athlete community.
            </p>
      ...
      {step === 'community' &&
        stepShell(
          <>
            <h2 className="font-serif text-2xl text-sand italic mb-2">You don't have to do this alone</h2>
            <p className="text-[13px] text-text-secondary mb-6">
            ...
            <div className="space-y-3">
              ...
              >
                ...
                >
                  {community === 'join' && <div className="w-2.5 h-2.5 rounded-full bg-volt" />}
                </div>
                <span className="flex-1">
                  <span className="text-[15px] block">Count me in</span>
                  <span className="text-[13px] text-text-tertiary block mt-0.5">
                    You may be asked to verify your athlete status before posting.
                  </span>
                </span>
              </button>
              <button
              ...
              >
                ...
                >
                  {community === 'solo' && <div className="w-2.5 h-2.5 rounded-full bg-volt" />}
                </div>
                <span className="text-[15px]">I prefer to go through the program on my own for now.</span>
              </button>
            </div>
            {backNextRow(
      ...
      {step === 'complete' && (
        <div className="w-full max-w-xl animate-rise">
          <div className="bg-bg-surface rounded-[20px] border border-border-subtle p-8 md:p-10 grain text-center">
            <p className="font-mono text-[11px] uppercase tracking-widest text-volt mb-4">Day 1 / 90 · Foundation</p>
            <h1 className="font-serif text-4xl md:text-5xl text-sand italic mb-6">You're in the game.</h1>
