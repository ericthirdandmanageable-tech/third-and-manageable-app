...
import { ChevronLeft, ArrowBigUp, MessageSquare, Flame, Clock, Trophy, Plus } from 'lucide-react';
import clsx from 'clsx';
import { api } from '../lib/api';
import { getForum, getPostsForForum, FLAIR_STYLES, CATEGORY_STYLES, type ForumPost, type PostFlair } from '../data/community';
...
const Forum = () => {
  ...
  const [posts, setPosts] = useState<ForumPost[]>(threadId ? getPostsForForum(threadId) : []);
  // Artifact "Share to forum" lands here with a prefilled draft
  const draft = (location.state as { draft?: { title: string; body: string } } | null)?.draft;
  const [composing, setComposing] = useState(Boolean(draft));

  const forum = getForum(threadId);

  const fetchPosts = async (s: Sort) => {
    if (!threadId) return;
    const remote = await api.getPosts(threadId, s);
    if (remote) {
      setPosts(remote.map((p) => ({
        id: String(p.id), threadId: p.forum_id, author: p.author_name, flair: p.flair as PostFlair,
        title: p.title, body: p.body, upvotes: p.upvotes, commentCount: p.comment_count, timeAgo: p.time_ago,
      })));
    }
  };

  useEffect(() => { fetchPosts(sort); }, [sort, threadId, user]); // eslint-disable-line
  ...
  if (!forum || !threadId) {
    return (
      <div className="p-6 md:p-10 max-w-3xl mx-auto text-center pt-24 animate-rise">
        ...
        <p className="text-[14px] text-text-secondary mb-6">That forum doesn't exist — it may have been renamed.</p>
        <button onClick={() => navigate('/community')} className="text-volt text-[14px] hover:underline underline-offset-4">
    ...
    <div className="p-6 md:p-10 max-w-3xl mx-auto animate-rise">
      ...
      <header className="mb-6">
        <div className="flex items-center gap-4 mb-2">
          <div className={`w-12 h-12 rounded-lg flex items-center justify-center ${CATEGORY_STYLES[forum.category].tile}`}>
          ...
          <div>
            ...
            <p className="font-mono text-[11px] uppercase tracking-widest text-text-tertiary">
              <span className="text-volt">{forum.activeNow} online</span> · {forum.memberCount} members
            </p>
          </div>
        </div>
      ...
      {user ? (
        !composing ? (
          ...
          <Composer
            forumId={threadId}
            initial={draft}
            onDone={() => { setComposing(false); fetchPosts(sort); }}
            onCancel={() => setComposing(false)}
          />
        )
      ) : (
        <p className="text-[13px] text-text-tertiary mb-6">Sign in to post — verified athletes only.</p>
      )}
        ...
        {sortTabs.map((tab) => (
          <button
            ...
            className={clsx(
              ...
              sort === tab.id ? 'bg-volt text-volt-ink' : 'text-text-secondary hover:text-text-primary'
            )}
          >
      ...
      {sorted.length === 0 ? (
        <div className="bg-bg-surface rounded-2xl border border-border-subtle p-8 text-center">
          ...
          <p className="text-[14px] text-text-secondary">
            No posts yet{user ? ' — the first one usually breaks the ice for everyone.' : '.'}
          </p>
        </div>
      ) : (
        <div className="space-y-3">
          {sorted.map((post) => (
            <button
              key={post.id}
              onClick={() => navigate(`/community/${threadId}/${post.id}`)}
            ...
            >
              ...
              <p className="text-[13px] text-text-secondary line-clamp-2 mb-3">{post.body}</p>
...
const Composer = ({ forumId, initial, onDone, onCancel }: { forumId: string; initial?: { title: string; body: string }; onDone: () => void; onCancel: () => void }) => {
  ...
  return (
    <div className="bg-bg-surface rounded-2xl border border-border-subtle p-5 mb-6 animate-disclosure">
      ...
      <div className="flex gap-2">
        <button onClick={submit} disabled={busy} className="bg-volt text-volt-ink font-semibold px-5 py-2 rounded-full text-[14px] hover:bg-volt/90 disabled:opacity-50 transition-all">
          {busy ? 'Posting…' : 'Post'}
        </button>
        <button onClick={onCancel} className="px-5 py-2 rounded-full text-[14px] text-text-tertiary hover:text-text-secondary transition-colors">
          Cancel
        </button>
      </div>
    </div>
