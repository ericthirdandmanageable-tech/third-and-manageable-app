...
import { ChevronLeft, Clock, Wallet, Heart, AlertTriangle, Repeat, Users, Check } from 'lucide-react';
import { getPath } from '../data/paths';
import { getForum } from '../data/community';
  ...
  if (!path) {
    return (
      <div className="p-6 md:p-10 max-w-3xl mx-auto text-center pt-24 animate-rise">
        ...
        <p className="text-[14px] text-text-secondary mb-6">That work structure doesn't exist in the registry.</p>
        <button onClick={() => navigate('/game-plan')} className="text-volt text-[14px] hover:underline underline-offset-4">
  ...
  return (
    <div className="p-6 md:p-10 max-w-3xl mx-auto animate-rise">
      ...
      <header className="mb-8">
        <div className="flex items-center gap-4 mb-3">
          <div className="w-14 h-14 rounded-lg bg-volt/10 flex items-center justify-center shrink-0">
            <path.icon className="w-7 h-7 text-volt" />
          </div>
          <div>
            <h1 className="font-serif text-4xl text-sand italic">{path.name}</h1>
            <p className="font-mono text-[11px] uppercase tracking-widest text-text-tertiary">{path.meta}</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
        <div className="bg-bg-surface rounded-2xl border border-border-subtle p-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="font-mono text-[11px] uppercase tracking-widest text-text-tertiary">Schedule shape</h2>
          </div>
          <p className="text-[15px] text-text-primary leading-relaxed">{path.scheduleShape}</p>
        </div>
        <div className="bg-bg-surface rounded-2xl border border-border-subtle p-5">
          <div className="flex items-center gap-2 mb-3">
            <Wallet className="w-4 h-4 text-volt" />
            <h2 className="font-mono text-[11px] uppercase tracking-widest text-text-tertiary">Income texture</h2>
          </div>
          <p className="text-[15px] text-text-primary leading-relaxed">{path.incomeTexture}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
        <div className="bg-bg-surface rounded-2xl border border-border-subtle p-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="font-mono text-[11px] uppercase tracking-widest text-text-tertiary">What athletes love</h2>
          </div>
          <ul className="space-y-2">
            {path.loves.map((item) => (
              <li key={item} className="flex gap-2 text-[15px] text-text-primary leading-relaxed">
        ...
        <div className="bg-bg-surface rounded-2xl border border-border-subtle p-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="font-mono text-[11px] uppercase tracking-widest text-text-tertiary">What's hard about it</h2>
          </div>
          <ul className="space-y-2">
            {path.hates.map((item) => (
              <li key={item} className="flex gap-2 text-[15px] text-text-primary leading-relaxed">
      ...
      <section className="bg-bg-surface rounded-[20px] border border-border-subtle p-6 md:p-8 mb-8 grain">
        ...
        <p className="text-[13px] text-text-secondary mb-5">Low-risk ways to test this path before you commit.</p>
        <div className="space-y-3">
          {path.firstReps.map((rep, i) => (
            <div key={rep} className="flex items-center gap-4 bg-bg-elevated border border-border-subtle rounded-2xl px-5 py-4">
              <span className="font-mono text-[13px] text-volt shrink-0">{String(i + 1).padStart(2, '0')}</span>
