...
import { Flame, CheckSquare, MessageSquare, Target, ChevronRight } from 'lucide-react';
import clsx from 'clsx';
import { JOURNEY_PHASES, WEEKLY_ACTIONS } from '../data/journey';
import { useGamePlan } from '../hooks/useGamePlan';
import { useCheckIns } from '../hooks/useCheckIns';
import { useAuth } from '../lib/useAuth';
import { api, authStorage } from '../lib/api';
import { getPath } from '../data/paths';
import { SkillMapCard, DayCounterCard, PathCommitmentCard, WeeklyRecapCard } from '../components/artifacts';
...
const Progress = () => {
  ...
  useEffect(() => {
    (async () => {
      if (!authStorage.getToken()) {
        ...
        setSport(data.intakeAnswers?.sport ?? null);
        return;
      }
  ...
  const filledDays = useMemo(() => {
    ...
    return new Set(
      history.map((e) => Math.floor((new Date(e.date + 'T00:00:00').getTime() - t0) / 86_400_000) + 1),
    );
  }, [history]);
  ...
  return (
    <div className="p-6 md:p-10 max-w-3xl mx-auto animate-rise">
      ...
      <section className="bg-bg-surface rounded-[20px] border border-border-subtle p-6 mb-8 grain">
        <p className="font-mono text-[10px] uppercase tracking-widest text-text-tertiary mb-3">The transition</p>
            ...
            return (
              <div key={m.day} className="flex flex-col items-center gap-1.5">
                <div className={clsx(
                  ...
                  reached ? 'bg-volt border-volt' : 'bg-bg-elevated border-text-tertiary',
                )} />
                <span className={clsx(
                  'font-mono text-[10px] uppercase tracking-widest text-center',
                  reached ? 'text-volt' : 'text-text-tertiary',
                )}>
                  Day {m.day}<br />{m.label}
                </span>
              </div>
            );
          })}
        </div>
        <div className="relative h-1 bg-bg-elevated rounded-full mt-[-52px] mb-8 mx-2 -z-0" aria-hidden>
          <div
            className="h-full bg-volt/60 rounded-full transition-all"
            style={{ width: `${Math.min((dayNumber / data.totalDays) * 100, 100)}%` }}
          />
        </div>
      </section>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
        {stats.map((stat) => (
          <div key={stat.label} className="bg-bg-surface rounded-2xl p-4 border border-border-subtle text-center">
            <stat.icon className="w-5 h-5 text-volt mx-auto mb-2" />
            <p className="font-mono text-2xl text-text-primary">{stat.value}</p>
            <p className="font-mono text-[10px] uppercase tracking-widest text-text-tertiary mt-1">{stat.label}</p>
          </div>
        ))}
      </div>
      ...
      <section className="bg-bg-surface rounded-2xl border border-border-subtle p-5 mb-8">
        <h2 className="font-mono text-[11px] uppercase tracking-widest text-text-tertiary mb-4">The 90-day arc</h2>
        <div className="flex gap-6 overflow-x-auto pb-2 snap-x snap-mandatory">
          {phaseDays.map((ph) => (
            <div key={ph.id} className="shrink-0 snap-start">
              <p className="font-mono text-[10px] uppercase tracking-widest text-text-tertiary mb-3">{ph.name}</p>
                ...
                {ph.days.map((day) => {
                  ...
                  return (
                    <div
                      ...
                      className={clsx(
                        'w-4 h-4 rounded-[4px]',
                        filled ? 'bg-volt' : day === dayNumber ? 'bg-volt/30 border border-volt/60' : 'bg-bg-elevated',
                      )}
                    />
                  );
                })}
        ...
        <p className="font-mono text-[11px] text-text-tertiary mt-4">
          {loading ? '…' : (
            <>
              <span className="text-volt">{history.length} {history.length === 1 ? 'day' : 'days'} showed up</span>
        ...
        {!loading && history.length === 0 && (
          <p className="text-[13px] text-text-secondary mt-2">
            The arc is empty because it hasn't started. Today's check-in is Day 1.
          </p>
        )}
      </section>
          ...
          <div className="space-y-2">
            {recentMoods.map((entry) => (
              <div key={entry.date} className="flex items-baseline gap-3">
                ...
                <span className="text-[13px] text-text-secondary">{entry.option}</span>
              </div>
            ))}
          </div>
        </section>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          <DayCounterCard day={dayNumber} totalDays={data.totalDays} phaseName={data.phase.name} />
          <WeeklyRecapCard streak={streak} completed={data.completedActionIds.length} total={data.weeklyActions.length || WEEKLY_ACTIONS.length} />
          {data.intakeDone && <SkillMapCard entries={data.skillMap} />}
          {data.committedPathId && <PathCommitmentCard pathId={data.committedPathId} entries={data.skillMap} day={dayNumber} />}
        </div>
      </section>
    </div>
  );
