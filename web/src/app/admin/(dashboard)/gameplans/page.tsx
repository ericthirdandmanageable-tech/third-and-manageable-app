...
import { listAdminActionCompletions } from "@/lib/admin-data";
import { CalendarDays, ClipboardList, Star, Users } from "lucide-react";

const ACTION_LABELS: Record<string, string> = {
  "career-explore": "Career Exploration",
  "career-network": "Networking",
  "career-resume": "Resume & LinkedIn",
  "routine-morning": "Morning Routine",
  "routine-exercise": "Exercise",
  "routine-sleep": "Sleep Hygiene",
  "mindset-journal": "Journaling",
  "mindset-gratitude": "Gratitude",
  "mindset-meditation": "Meditation",
  "social-connect": "Social Connection",
  "social-mentor": "Mentorship",
  "social-community": "Community",
  "wellness-therapy": "Therapy",
  "wellness-nutrition": "Nutrition",
  "wellness-hobby": "New Hobby",
};

async function getGamePlanData() {
  const all = await listAdminActionCompletions();

  const actionCounts: Record<string, number> = {};
  all.forEach((c) => {
    actionCounts[c.action_id] = (actionCounts[c.action_id] || 0) + 1;
  });
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    dailyCounts.push({
      date: dateStr,
      count: all.filter((c) => c.date === dateStr).length,
    });
  }
  ...
  return {
    ...
    sortedActions,
    dailyCounts,
    mostPopular: sortedActions[0]?.label || "N/A",
  };
}
...
export default async function GamePlansPage() {
  ...
  const maxCount =
    data.sortedActions.length > 0 ? data.sortedActions[0].count : 1;
          ...
          <div className="space-y-3">
            {data.sortedActions.slice(0, 8).map((a) => (
              <div key={a.id} className="flex items-center gap-2 sm:gap-3">
                <span className="text-xs sm:text-sm text-gray-400 w-28 sm:w-36 truncate">
                  {a.label}
                </span>
