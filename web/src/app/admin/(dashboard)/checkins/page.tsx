...
import { listAdminCheckIns } from "@/lib/admin-data";
...
async function getCheckInData() {
  const allCheckins: CheckIn[] = await listAdminCheckIns();
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    dailyCounts.push({
      date: dateStr,
      count: allCheckins.filter((c) => c.date === dateStr).length,
    });
  }
          ...
          <table className="w-full min-w-[500px]">
            <thead>
              <tr className="border-b border-gray-800">
                <th className="text-left px-4 sm:px-6 py-3 text-xs sm:text-sm font-medium text-gray-400">
                  User
                </th>
                <th className="text-left px-4 sm:px-6 py-3 text-xs sm:text-sm font-medium text-gray-400">
                  Mood
            ...
            <tbody>
              {data.recent.map((c) => (
                ...
                >
                  <td className="px-4 sm:px-6 py-3 text-xs sm:text-sm text-white">
                    {c.display_name}
                  </td>
                  <td className="px-4 sm:px-6 py-3">
                    <span className={`text-sm ${MOOD_COLOR[c.mood] || ""}`}>
                      {c.mood}/5
                    </span>
                  </td>
                  <td className="px-4 sm:px-6 py-3 text-xs sm:text-sm text-gray-300 max-w-[200px] truncate">
                    {c.note || "—"}
                  </td>
                  <td className="px-4 sm:px-6 py-3 text-xs text-gray-400">
                    {c.created_at
